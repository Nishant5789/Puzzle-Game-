...
var screen = document.getElementById('img-main'); // take the reference of html tags
...
//async is used to deal with promice
...
const board_start = (index) => {
    ...
    for (let r=0; r < rows; r++) {
        for (let c=0; c < columns; c++)
        {
            ...
            tile.style.width="120x";
            tile.style.height="120px";
        
            tile.style.border="2px solid #0c67ae";
            tile.id = r.toString()+ "-" +c.toString();

            tile.src = `level/lavel_${index}/${imgOrder[update_index]}.png`;
...
var dragEnd = (index) => {
    if (!otherTile.src.includes(`level/lavel_${index}/9.png`)) {
        return;
    }
    ...
    if (isAdjacent) 
    {
        ...
        if(JSON.stringify(imgOrder)==JSON.stringify(originalorder))
        {
            ...
            else if(turns>5 && turns<10)
            {
                ...
                comment_text.innerText="Good Hope next time Improve Your Game";
...
const board = (index)=> {
    console.log("board");
    screen.style.display="none";
    puzzle_screen.style.display="flex";
    board_start(index);
}
...
const level = (flag) => {
    ...
    {
        result_screen.style.display="none";
        level_screen.style.display="flex";
    }
    else
    {
        screen.style.display="none";
